...
const PersonalizedFeedbackReport: React.FC<PersonalizedFeedbackReportProps> = ({ feedback, isGeneratingFeedback, onStartFocusedQuiz }) => {
    if (isGeneratingFeedback && (!feedback || Object.keys(feedback).length === 0)) {
        return (
            <div className="w-full max-w-md lg:max-w-5xl bg-surface-dark p-6 rounded-2xl shadow-2xl">
                <div className="flex flex-col items-center justify-center text-center">
                    <LoadingSpinner />
                    <h3 className="text-xl font-bold text-text-primary mt-4">AI Coach is available...</h3>
                    <p className="text-text-secondary mt-1">Generating feedback based on performance.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-md lg:max-w-5xl bg-surface-dark p-6 sm:p-8 rounded-2xl shadow-2xl text-left animate-fade-in">
            ...
            {overallSummary ? (
                ...
                isGeneratingFeedback && <div className="mb-6"><SectionLoadingSpinner /></div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 my-6 lg:items-start">
                <div>
                    ...
                    ) : (isGeneratingFeedback && <SectionLoadingSpinner />)}
                </div>
