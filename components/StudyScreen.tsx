...
const StudyScreen = ({
    ...
    const gradeAndFinalizeAnswer = useCallback(async (
        ...
        const { matchResults = null, sequenceResults = null } = options;
        // Bypass if explicitly requested OR if the global setting disables it
        const bypassConfidence = options.bypassConfidence ?? (quizConfig?.enableConfidenceCheck === false);
    ...
    }, [answerStatus, streak, timeLeft, isUntimedMode, currentQuestion, history, commitResult, quizConfig]);
                                ...
                                {promptItems.map((p) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`p-4 rounded-lg border-2 transition-all duration-200 ${p.isPlaced ? 'bg-surface-dark border-gray-700 text-gray-500 cursor-not-allowed' :
                                ...
                                {matchingQ.answers.map((answerText, answerIndex) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`p-4 rounded-lg border-2 min-h-[80px] flex flex-col justify-center transition-all duration-200 cursor-pointer ${resultState === 'correct' ? 'border-correct bg-correct/10' :
                                        ...
                                        >
                                            ...
                                            {placedPrompt ? (
                                                <div
                                                    ...
                                                    className={`mt-2 p-2 rounded-md text-center text-white font-semibold ${resultState === 'correct' ? 'bg-correct/80' :
