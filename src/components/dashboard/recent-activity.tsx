import { Microscope, PlayCircle, Network, Atom, ChevronRight, BookOpen } from 'lucide-react';
import Link from 'next/link';

export function RecentActivity({ studySets = [] }: { studySets?: any[] }) {
    return (
        <div className="col-span-8 space-y-4">
                    ...
                    <div className="flex justify-between items-start mb-4 relative z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span className="text-xs font-mono text-gray-500">{studySets[0].description || 'General'} • {_countMsg(studySets[0])}</span>
                            </div>
                        </div>
                        <div className="text-brand-success text-lg font-bold">0%</div>
                    </div>

                    <div className="w-full h-1 bg-white/5 rounded-full mb-4 overflow-hidden">
                        <div className="h-full bg-brand-success w-[0%] rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]"></div>
                    </div>

                    <div className="flex gap-3 relative z-10">
                        <Link href={`/study/${studySets[0].id}`} className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-sm flex items-center gap-2 border border-white/5 text-gray-300 hover:text-white transition">
            ...
            {/* List Items */}
            {studySets.slice(1).map((set) => (
                <Link key={set.id} href={`/study/${set.id}`} className="block">
                    <div className="glass p-4 rounded-xl flex items-center justify-between hover:translate-x-1 transition-all cursor-pointer group mb-2">
                        ...
                        <div className="text-right flex items-center gap-4">
                            <div className="text-sm text-gray-400">{_countMsg(set)}</div>
...
function _countMsg(set: any) {
    const m = set._count?.materials || 0;
    return `${m} Material${m !== 1 ? 's' : ''}`;
}
